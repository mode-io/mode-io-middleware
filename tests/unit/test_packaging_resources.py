...
class TestPackagingResources(unittest.TestCase):
    def test_bundled_resources_exist(self):
        ...
        self.assertTrue(bundled_dashboard_index_path().exists())
        self.assertTrue(bundled_dashboard_favicon_path().exists())
        ...
        self.assertTrue(
            (
                bundled_protocol_schema_dir() / "MODEIO_PLUGIN_MANIFEST.schema.json"
            ).exists()
        )
        self.assertTrue(
            (
                bundled_protocol_schema_dir() / "MODEIO_PLUGIN_MESSAGE.schema.json"
            ).exists()
        )

    def test_repo_and_bundled_default_configs_match(self):
        repo_payload = json.loads(
            (PACKAGE_ROOT / "config" / "default.json").read_text(encoding="utf-8")
        )
        ...
        self.assertTrue(
            all(
                profile.get("plugins") == []
                for profile in repo_payload["profiles"].values()
            )
        )

    def test_repo_and_bundled_example_manifest_match(self):
        repo_payload = json.loads(
            (PACKAGE_ROOT / "plugins_external" / "example" / "manifest.json").read_text(
                encoding="utf-8"
            )
        )
        bundled_payload = json.loads(
