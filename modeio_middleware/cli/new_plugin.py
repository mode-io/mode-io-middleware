...
class Test{class_name}ProtocolPlugin(unittest.TestCase):
    def test_manifest_is_present(self):
        manifest_path = Path(__file__).resolve().parents[1] / "plugins_external" / "{module_name}" / "manifest.json"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default=".",
        help="Directory where plugin/test files should be created (default: current directory)",
    )
