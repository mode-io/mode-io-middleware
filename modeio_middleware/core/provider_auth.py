...
def _openclaw_models_cache_inspection(
    provider_id: str, env: Mapping[str, str]
) -> CredentialInspection | None:
    payload = _read_json_object(_openclaw_models_cache_path(env))
    if payload is None:
        return None
...
def _inspect_openclaw_provider(
    ...
    if selection.reason.startswith("fallback_provider:") and isinstance(selection.credential, dict):
        ...
        return _ready(
            ...
            path=str(_openclaw_models_cache_path(env)),
            authorization=_bearer(str(selection.credential.get("apiKey") or "")) or "",
            reason=(
    ...
    if selection.profile_id is None or not isinstance(selection.credential, dict):
        return _missing(
            ...
            else f"no reusable OpenClaw auth profile found for provider '{current_provider}'",
            fallback_mode=FALLBACK_MODE_MANAGED_UPSTREAM,
            metadata={"providerId": current_provider, "selectionReason": selection.reason},
        )
    ...
    inspection = _openclaw_models_cache_inspection(current_provider, env)
    if inspection is not None:
        return inspection
    ...
    if env_key:
        return _ready(
            ...
            authorization=_bearer(env_key) or "",
            metadata={"providerId": current_provider},
        )
    ...
    return _missing(
        ...
        strategy="missing",
        reason=(
            f"OpenClaw provider '{current_provider}' has no reusable auth profile, cache API key, or OPENAI_API_KEY fallback"
        ),
        fallback_mode=FALLBACK_MODE_MANAGED_UPSTREAM,
        metadata={"providerId": current_provider},
    )
